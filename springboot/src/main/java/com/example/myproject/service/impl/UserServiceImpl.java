...
import com.example.myproject.common.RoleEnum;
...
import com.example.myproject.exception.ServiceException;
import com.example.myproject.mapper.RoleMapper;
import com.example.myproject.mapper.RoleMenuMapper;
...
//        这里MD5加密了，传递的密码是123456，数据库的密码也是123456，但是这里MD5加密后的密码是其他的（不是123456），所有根据sql就查不到用户
//        解决方案有两种，一种是改数据库（建议，将数据库的密码，改为md5加密后的密码），一种是改代码（下面写的这个）
    ...
    public User register(UserDTO userDTO) {
        ...
        if (one == null) {
            one = new User();
            //BeanUtil.copyProperties(userDTO, one, true);  //hutool工具类有bug
